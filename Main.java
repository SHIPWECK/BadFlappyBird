...
class FlappyGame implements KeyListener {
    ...
    protected final int PIPE_WIDTH = 50,
            PIPE_HEIGHT = 500,
            PIPE_OFFSET = 150,
            FLAPPY_WIDTH = 35,
            FLAPPY_HEIGHT = 9;

    protected JPanel topPipe1,
            bottemPipe1,
            topPipe2,
            bottemPipe2,
            topPipe3,
    ...
    FlappyGame() {
        ...
        for (int i = 0; i < allPipes.length; i++) {
            ...
            // sees if the pipe is top or bottom and sets its position to be either on top
            // or on the bottom.
            if (i % 2 == 0)
                // sets the X of the pipe slightly offscreen and the top pipe a sligtly
                // randomized distance
                allPipes[i].setBounds(gameWindow.getWidth() + (PIPE_WIDTH + 30) * i, rand.nextInt(-300, -150),
                        PIPE_WIDTH, PIPE_HEIGHT);
            else
                // sets the X the same way and sets the y to the top pipe minus an offset
                // constant
                allPipes[i].setBounds(allPipes[i - 1].getX(), allPipes[i - 1].getY() + PIPE_HEIGHT + PIPE_OFFSET,
                        PIPE_WIDTH, PIPE_HEIGHT);
    ...
    public void keyTyped(KeyEvent e) {
        switch (e.getKeyChar()) {
            // jump
            case ' ':
                // resets the flappy bird position to before the peak of its projectile
                // parabola,
                // simulating a jump
                parbolaPositionX = 0;
                break;
                ...
                for (int i = 0; i < allPipes.length; i++)
                    if (i % 2 == 0)
                        if (inBounds(allPipes[i]))
                            ...
                            // sends the pipes off screen to the right after they've left the left side of
                            // the window
                        ...
                        allPipes[i].setLocation(allPipes[i - 1].getX(),
                                allPipes[i - 1].getY() + PIPE_HEIGHT + PIPE_OFFSET);
            }
        };
    ...
    protected void endGame() {
        ...
        endMenu.setLocation(gameWindow.getWidth() / 2 - endMenu.getWidth() / 2 - 5,
                gameWindow.getHeight() / 2 - endMenu.getHeight() / 2);
